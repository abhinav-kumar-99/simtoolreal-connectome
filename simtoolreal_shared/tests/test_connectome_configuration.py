...
def test_release_settings_suite_preserves_original_logical_update_schedule() -> None:
    ...
    assert training["actor_microbatch_size"] == 12288
    assert training["central_critic_microbatch_size"] == 12288
...
def test_release_settings_suite_routes_microbatches_without_changing_logical_batch(
    ...
    assert "++train.params.config.microbatch_size=12288" in overrides
    assert (
        "++train.params.config.central_value_config.microbatch_size=12288"
        in overrides
    )
