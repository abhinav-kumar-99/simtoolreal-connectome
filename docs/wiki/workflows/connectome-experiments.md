...
The fresh full no-auxiliary job runs in tmux `connectome-fixed-reservoir-gaussian-noaux-100b` as suite PID 430585 and trainer PID 430658 on physical GPU 1. Its resolved configuration confirms `activation: tanh`, `use_experimental_cv: false`, the fixed map and cached motor readout, clipped coefficient-conditioned Gaussian capped at sigma three, K=4, LF/1.0, entropy scale `.005`, actor KL `.004`, privileged central critic and 100B cap. At epoch 4/frame 589,824 it was live at 144,621 frames/s with a `.243`-second update. Watcher PID 431147 runs in `connectome-fixed-reservoir-gaussian-noaux-videos`, awaiting 250M-frame milestones. A new symlink exposes the run to the existing port-6008 TensorBoard without restarting the server. The LIF job is not running.
