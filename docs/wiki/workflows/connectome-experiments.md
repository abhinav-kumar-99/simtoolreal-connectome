...
### Staged 1G success handoff: 5x to 3x to capped Gaussian

The corrected handoff does not replace the active 5x run immediately. It first waits until that run has emitted a `mean_successes/frame` point at or above 1,000,000,000 TensorBoard frames. At that time it compares the closest logged point to 1G against the closest point from the existing 1x LF reference. A tie in distance selects the lower frame. Replacement uses strict less-than: 5x is stopped only if its selected success value is below 1x. If it is equal or higher, 5x is retained and the chain terminates.

If 5x loses, the monitor stops only its exact trainer, suite and video-watcher process trees and launches a fresh restricted-Beta 3x job plus its watcher on GPU 0. The new job starts from frame zero in a distinct output directory; the monitor then independently waits for that 3x job to cross 1G. It compares 3x to the same fixed 1x-at-1G reference. Only if 3x is also lower does it stop 3x and launch the terminal clipped-Gaussian fallback. The GPU-1 1x reference is never stopped by this monitor. At monitor creation, the selected reference was frame 999,948,288 with `mean_successes/frame = 0.0069173179`; 5x had reached only frame 293,535,744 and therefore had no decision yet.
...
The handoff YAML owns the metric tag, 1G target, point-selection/comparison rules, exact event globs, exact process markers, stage order, replacement suite/video configs, poll cadence and shutdown/startup timeouts. The monitor incrementally scans complete TensorBoard records, persists offsets/bracketing points, decisions and launched/stopped PIDs in `train_dir/connectome/adaptation_100b_gains_update_timing/lf_entropy_success_handoff/status.json`, and calls the existing suite and milestone-evaluation entrypoints. `run_connectome_numerical_fallback.py` supplies shared atomic-status, process-tree and subprocess-launch helpers; it is not separately started for this handoff.
...
The terminal Gaussian fallback retains that same 3x weighting and all other matched settings. Its `SimToolRealConnectome1952AdaptersMLPGaussianSigma3SAPG` profile applies a smooth ceiling to each emitted standard deviation: raw log standard deviation zero still maps to sigma one, and sigma approaches but cannot exceed three with a nonzero gradient below the asymptote. Existing Gaussian profiles omit `max_sigma` and are unchanged. This is a per-coordinate cap: 29 sigmas can sum to at most 87, but 87 is not an entropy bound. At sigma three in every dimension, the unclipped diagonal Gaussian entropy is about 73.01 nats; environment actions are still hard-clipped to `[-1,1]`. The direct suite and watcher configs are `ppo_1952_4update_gaussian_lf_entropy3x_sigma3_100b.yaml` and its matching `_milestones.yaml`, but the handoff launches them only after both Beta candidates lose their own 1G comparisons.

The monitor was launched in tmux session `connectome-lf-entropy-success-handoff` with PID 35283. Its first persisted status was `monitoring`: it found all three expected 5x process PIDs 9553/9610/9904, selected the fixed 1x reference above, and recorded the then-current 5x point at frame 304,152,576/value .0006510417 without making a comparison because no at-or-above-1G 5x point existed. The 1x trainer/watcher PIDs 4102915/4103035 also remained live.
