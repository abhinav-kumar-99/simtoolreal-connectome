...
### Staged 1.25G success handoff: 5x to 3x to capped Gaussian

The corrected handoff does not replace the active 5x run immediately. It first waits until that run has emitted a `mean_successes/frame` point at or above 1,250,000,000 TensorBoard frames. At that time it compares the closest logged point to 1.25G against the closest point from the existing 1x LF reference. A tie in distance selects the lower frame. Replacement uses strict less-than: 5x is stopped only if its selected success value is below 1x. If it is equal or higher, 5x is retained and the chain terminates.

If 5x loses, the monitor stops only its exact trainer, suite and video-watcher process trees and launches a fresh restricted-Beta 3x job plus its watcher on GPU 0. The new job starts from frame zero in a distinct output directory; the monitor then independently waits for that 3x job to cross 1.25G. It compares 3x to the same fixed 1x-at-1.25G reference. Only if 3x is also lower does it stop 3x and launch the terminal clipped-Gaussian fallback. The GPU-1 1x reference is never stopped by this monitor. The earlier 1G contract was superseded before any decision or transition occurred.
...
The handoff YAML owns the metric tag, 1.25G target, point-selection/comparison rules, exact event globs, exact process markers, stage order, replacement suite/video configs, poll cadence and shutdown/startup timeouts. The monitor incrementally scans complete TensorBoard records, persists offsets/bracketing points, decisions and launched/stopped PIDs in `train_dir/connectome/adaptation_100b_gains_update_timing/lf_entropy_success_handoff/status.json`, and calls the existing suite and milestone-evaluation entrypoints. If the YAML target changes while still monitoring and before any transition, it archives the prior selected points and clears all target-dependent brackets/offsets so both event histories are rescanned for the new target. `run_connectome_numerical_fallback.py` supplies shared atomic-status, process-tree and subprocess-launch helpers; it is not separately started for this handoff.
...
The original monitor launch used tmux session `connectome-lf-entropy-success-handoff` and PID 35283. Its initial 1G state was superseded before any decision or transition. The restarted monitor in the same session uses PID 38375. At the revised 1.25G target, the fixed reference selects frame 1,250,033,664 with `mean_successes/frame = 0.0086263027`; the 5x run was only at frame 343,867,392 and therefore still had no selected comparison point. The restart archived the old 1G bracket summary, rescanned both event histories, and left both trainers/watchers live.
