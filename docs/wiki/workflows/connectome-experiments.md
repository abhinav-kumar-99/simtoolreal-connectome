...
The original monitor launch used tmux session `connectome-lf-entropy-success-handoff` and PID 35283. Its initial 1G state was superseded before any decision or transition. At the revised 1.25G target, the fixed reference selects frame 1,250,033,664 with `mean_successes/frame = 0.0086263027`; the 5x run was only near 339M frames and therefore still had no selected comparison point. The monitor restart preserves the session name, archives the old 1G bracket summary, rescans both event histories, and leaves both trainers/watchers live.
