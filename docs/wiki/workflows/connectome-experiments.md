...
The original point-metric monitor launches were superseded before any decision or transition. Under the windowed true-objective contract, the fixed 1x reference has 508 logged samples from frame 1,150,156,800 through 1,249,837,056 and mean `0.0000419941081`; its first at-or-above-target point is frame 1,250,033,664. The 5x run had not reached the start of the comparison window when this contract was installed, so it remained ineligible. The monitor restarted in tmux session `connectome-lf-entropy-success-handoff` as PID 41405, archived the old point-metric state, fully rescanned both histories, and left both trainers/watchers live.
