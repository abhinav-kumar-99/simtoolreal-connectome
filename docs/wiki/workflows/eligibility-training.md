...
## Other input-adapter learning options

These are proposals, not implemented or validated alternatives. A sensory-encoding objective could supervise adapter outputs from robot joint position/velocity/contact using explicit population tuning templates. This requires choosing and validating the robot-to-fly correspondence; matching a chosen code does not prove useful motor control. A teacher-policy objective could instead train adapters and readout to imitate competent robot actions from recorded trajectories, following the general [policy distillation](https://arxiv.org/abs/1511.06295) principle. That supplies dense action targets but still requires derivatives through the frozen recurrent circuit unless paired with another optimizer; short unrolls are approximate and must cover enough sensory-to-motor propagation. Teacher collection/training cost and deployment distribution shift remain relevant.

A reward-driven alternative is parameter-space perturbation: evaluate paired adapter perturbations over full rollouts and update toward higher return, as in [evolution strategies](https://arxiv.org/abs/1703.03864). This removes recurrent backpropagation and can capture delayed consequences through rollout returns, but may require many simulations. Restricting the adapter to a low-rank or anatomically grouped parameterization is a proposed way to reduce the search dimension, not a proven speedup here. Short-window backpropagation is another intermediate option: keep recurrent state across windows but detach its history at boundaries, learning adapters from either policy or supervised losses while accepting missing longer-range derivatives. Sensory pretraining followed by task-based refinement is a hypothesis motivated by the user's desired robot-to-fly interface, not evidence of transferred native dexterity.
...
The matched-cadence relaunch started on 2026-09-14 at 07:28 EDT in tmux sessions `connectome-eligibility-100b` and `connectome-eligibility-adapters-100b`; coordinators are PIDs 3431537/3431540 and trainers are 3431649/3431650. Watchers are PIDs 3431831/3431834 in corresponding `-eval` sessions. TensorBoard 6008 discovered both authoritative paths. Both emitted finite eligibility metrics on the 196,608-frame grid; an early audit observed adapter+gains at frame 589,824/update 1,536 and adapters-only at frame 786,432/update 2,048. Both watcher manifests were running with no failures and 400 expected targets, three videos per target. These are launch-health observations, not learning-quality results.

At the user's direction on 2026-09-14, both eligibility trainers, their suite coordinators and both milestone watchers were interrupted. All six PIDs exited; the concurrently running PPO trainers/watchers were untouched. The latest saved recovery checkpoints remain at epoch 960/frame 188,743,680 for adapters plus gains and epoch 1,040/frame 204,472,320 for adapters only. Neither run reached the first 250M inference milestone, so no milestone videos were completed. The coordinator timing files record the requested SIGINT as a nonzero child exit, and the now-unattended watcher manifests retain their last `running` value; neither should be interpreted as an active process. All logs and checkpoints are preserved.
