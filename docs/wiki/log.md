...
- Added `SimToolRealConnectomeProbabilisticSAPG`, the 1,952-cell smoke contract, all requested `probabilistic_plasticity/*` telemetry, sampled/MAP inference, a matched benchmark contract, and focused CPU/CUDA/two-rank tests. Fourteen new tests passed; legacy CPU and Triton regressions passed 60 and 12 tests respectively.
- The contention-sensitive RTX 4090 benchmark measured signed-LoRA versus independent probabilistic PPO throughput at 42,688 versus 38,356 observations/s. Group size 32 measured 37,086 observations/s, so the default remains independent group size 1. Detailed equations, implementation boundaries, memory/timing values, and caveats are recorded in `docs/wiki/analyses/probabilistic-plasticity.md`.
