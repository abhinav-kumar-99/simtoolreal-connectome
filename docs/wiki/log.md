...
## [2026-09-21] implement | Signed relative LoRA + plasticity diagnostics
...
## [2026-09-19] launch | Replace K=1 with adaptive K=2 using one-third LR bounds

- Preserved K=1 through final visible TensorBoard frame 280,559,616, its latest full checkpoint and completed 250M dual-tolerance evaluation, then stopped only trainer PID 386701 and its watcher. The original constant-LR K=2 trainer PID 3843064 and watcher PID 386401 remained live on physical GPU 1.
- Added a fresh YAML-owned K=2 adaptive run on physical GPU 0. It retains initial actor LR `1e-4`, KL target `.004` and rollout-level single-decision scheduling, while scaling the standard actor bounds exactly by one third to `[3.333333333e-7, 3.333333333e-4]`. The privileged critic remains independently fixed at `1e-4` because it has no adaptive min/max rule.
- Validated the resolved Hydra contract and passed all 37 focused KL/scheduler tests. Added a separate dual-tolerance 250M milestone watcher without changing the constant K=2 watcher or its existing output state.
- Launched adaptive K=2 trainer PID 610456 plus its watcher. At frame 1,769,472 it had reached the scaled maximum LR `3.3333333e-4`; scheduler KL was .0017586, aggregate KL .0010221, entropy 41.1705 and decision count exactly one. GPU UUID placement and TensorBoard port 6008 confirmed the adaptive run on physical GPU 0 and constant control on physical GPU 1.

## [2026-09-19] launch | Replace fixed K=2 with matched adaptive K=4

- Preserved the fixed-LR K=2 run through final visible TensorBoard frame 1,317,666,816, its epoch-5,000 full checkpoint and completed dual-tolerance evaluations through 1.25B, then stopped only trainer PID 3843064 and its watcher. Adaptive K=2 PID 610456 and watcher PID 610177 remained live on physical GPU 0.
- Added a fresh YAML-owned K=4 run on physical GPU 1 that matches adaptive K=2 in initial actor LR `1e-4`, bounds `[3.333333333e-7, 3.333333333e-4]`, rollout KL target `.004`, critic LR `1e-4`, batch geometry, LF/1.0, entropy scale, policy profile, task settings, seed and 100B budget. The intended contract difference is K=4 versus K=2.
- Resolved both Hydra contracts and verified the matched fields, then passed all 37 focused KL/scheduler tests. Added a separate K=4 dual-tolerance milestone watcher and output identity.
- Launched K=4 trainer PID 698894 and watcher PID 698624. At frame 1,769,472 it had reached the scaled maximum LR; scheduler KL was .0018239, aggregate KL .0010507, entropy 41.1736 and decision count exactly one. GPU UUID placement and TensorBoard port 6008 confirmed K=2 on physical GPU 0 and K=4 on physical GPU 1.

## [2026-09-19] launch | Replace K=4 with original-bounds adaptive K=2

- Preserved the one-third-bounds adaptive K=4 run through final visible TensorBoard frame 673,382,400, its latest full checkpoint and completed dual-tolerance evaluations through 500M, then stopped only trainer PID 698894 and watcher PID 698624. The one-third-bounds K=2 trainer PID 610456 and watcher PID 610177 remained live on physical GPU 0.
- Added a fresh YAML-owned K=2 run on physical GPU 1. It restores the original actor LR bounds `[1e-6, 1e-3]` while retaining initial LR `1e-4`, rollout KL target `.004`, one adaptive decision per rollout, critic LR `1e-4`, batch geometry, LF/1.0, entropy scale, policy profile, task settings, seed and 100B budget.
- Validated that the new suite differs behaviorally from the live K=2 only in actor LR bounds, resolved its Hydra contract, and passed all 37 focused KL/scheduler tests. Added a separate dual-tolerance 250M milestone watcher and output identity.
- Launched original-bounds K=2 trainer PID 1296249 and watcher PID 1295977. At frame 1,769,472 its actor LR was `7.59375e-4`; scheduler KL was .0027973, aggregate KL .0017294, entropy 41.1876 and decision count exactly one. GPU UUID placement and TensorBoard port 6008 confirmed the one-third-bounds K=2 on physical GPU 0 and original-bounds K=2 on physical GPU 1.

## [2026-09-20] resume | Replace original-bounds K=2 with matched LSTM continuation

- Preserved the original-bounds adaptive K=2 through final visible TensorBoard frame 4,805,492,736, its latest full checkpoint and completed dual-tolerance milestones through 4.75B, then stopped trainer PID 1296249 and its matching watcher PID 1295977. The one-third-bounds K=2 trainer PID 610456 and watcher PID 610177 remained live on physical GPU 0.
...
## [2026-09-20] resume | Replace LSTM continuation with matched fly continuation

- Stopped only the resumed LSTM trainer PID 906066 and its LSTM-only video watcher PID 999563, preserving their checkpoint, appended TensorBoard events, and completed video tree. Its final visible TensorBoard frame was 3,456,958,464.
- Added YAML-owned fly continuation and fly-only milestone watcher contracts on physical GPU 1. The trainer restores the fly `last/model.pth` at epoch 12,800/frame 2,516,582,400 with `resume_training_state`, preserves the original rollout-KL/LF/batch/task/critic/100B contract, and targets the original experiment and `rl_runs` directories so new TensorBoard events append to the existing fly identity.
- Resolved the composed contract to verify the fly K=4 update count and existing artifact route, and passed all 37 focused KL/scheduler tests. The watcher reuses the fly status/output tree, retains ten complete targets through 2.5B, and will render the next 2.75B target as six mean-action videos.
- Launched trainer PID 1300720 in tmux `connectome-fly1952-rollout-kl-resume` and watcher PID 1300546 in `connectome-fly1952-rollout-kl-resume-eval`. The initial continuation advanced beyond frame 2.525B with finite rollout KL, one scheduler decision, and a second event file in the original fly summaries directory. The independent one-third-bounds K=2 trainer/watcher on GPU 0 remained live.

## [2026-09-20] launch | Replace K=4 fly continuation with K=2 intrinsic plasticity

- Stopped only the K=4 resumed-fly suite/trainer process group (including trainer PID 1300720) and its matching watcher process group (including watcher PID 1300546). Their existing checkpoint, TensorBoard-event, and milestone-video artifacts were preserved. The independent non-plastic K=2 suite/trainer/watcher on GPU 0 remained live.
- Moved the pre-existing YAML-owned K=2 intrinsic-plasticity suite and matching watcher from GPU 0 to the newly freed physical GPU 1, avoiding collision with the non-plastic control. The fresh suite uses `neural_updates: 2`, `adapters_only`, and `learn_dynamics: true`, so it trains leak, recurrent bias, and per-neuron intrinsic gain while retaining fixed connectome edge weights and neuron gains.
- Validated suite/watcher YAML consistency and passed `test_intrinsic_gain_identity_trainability_and_checkpoint`. Launched trainer PID 1907621 in tmux `connectome-k2-intrinsic-plasticity` and watcher PID 1909557 in `connectome-k2-intrinsic-plasticity-eval`. At epoch 17/frame 3,145,728 it reported finite values and 115,741 total FPS; this is launch health only.

## [2026-09-20] implement | Register all suite-launched jobs on TensorBoard 6008

- The persistent TensorBoard process on port 6008 serves `train_dir/connectome/adaptation_100b_gains_update_timing`. Registered the already-running K=2 intrinsic-plasticity summary directory there and confirmed discovery through the local `/data/runs` API.
- `scripts/run_connectome_suite.py` now registers every training case before launching its child trainer. It creates a suite-qualified symlink to that case's `summaries/` directory, defaults to the port-6008 aggregation root, permits an explicit YAML `training.tensorboard_log_root` override, and rejects a colliding name that points elsewhere.
- Added focused registration/reuse/collision tests; both passed. This affects future YAML suite launches, including helper scripts that invoke the suite launcher.

## [2026-09-21] implement | Spectral gradient preconditioning for signed LoRA

- Added detached full-SVD gradient preconditioning for signed low-rank `edge_u`/`edge_v` only. The forward map stays \(W^{\mathrm{eff}}=W^0(1+\Delta)\) on existing edges, and the plasticity loss stays \(\lambda_{\mathrm{syn}}\mathrm{mean}(\Delta^2)\). Adam still owns `U` and `V`; moments are not reset when the basis refreshes.
- The basis is fixed for one PPO update (rollout plus all mini-epochs) and recomputed from the SpMM `W_eff` afterward. Multipliers are \((\max(\sigma, c\sigma_{\max}))^\alpha\) normalized to mean 1. New TensorBoard tags are `spectral_preconditioning/*`. Defaults remain off.
- Replaced the K=4 intrinsic-only suite and milestone watcher with a GPU-1 copy of the rank-4 synaptic job at `alpha: -0.5`. The GPU-0 synaptic-only control and the combined intrinsic+synaptic suite are unchanged.
- The spectral suite sets `training.tensorboard_log_root: train_dir/tensorboard_6008`, matching the live port-6008 process. The suite launcher symlinks that run's `summaries/` directory there when training starts.
- Stopped the synaptic-only suite/trainer/watcher (PIDs 2849850/2850215/2849849) and left the intrinsic+synaptic job on GPU 1. Moved the spectral suite to physical GPU 0 and launched suite/trainer/watcher PIDs 429543/429846/429539. Port 6008 lists the new summaries symlink.
- Switched the preconditioner SVD from float64 to float32 after the first run spent about 15–21 seconds in each update. Archived that partial tree as `..._fp64svd_partial`, stopped suite/trainer/watcher PIDs 429543/429846/429539, and relaunched suite/trainer/watcher PIDs 487706/488100/487707 on GPU 0. By epoch 15 the update time was about 1.9–2.1 s and total FPS about 55–60k. The GPU-1 intrinsic+synaptic trainer PID 2850208 stayed up.
- Measured the anatomical spectrum: 1,361 of 1,952 singular values exceed `1e-3 * sigma_max` (3.643). A partial SVD at the current floor does not remove the dense factorization. bfloat16 SVD is unsupported (`svd_cuda_gesvdj`).
- The float32 spectral trainer died at epoch 200 because `last_<experiment>_ep_200_rew_....pth` was 279 bytes and the filesystem rejects basenames above 255. `save_checkpoint` now shortens overlong basenames.
- Stopped the resumed long-name trainer PID 642080 and watcher PID 487707. Restarted fresh, without a checkpoint, under experiment `00_spectral_r4_a-0p5_k4_seed42` (71-byte worst-case checkpoint basename). Suite/trainer/watcher PIDs 675324/675634/675325 on GPU 0. Epoch 1 began with no `checkpoint=` override. The GPU-1 intrinsic+synaptic trainer stayed up.
- That short-name run had `learn_dynamics: false`. Stopped suite/trainer/watcher PIDs 675324/675634/675325, archived the tree as `spectral_r4_a-0p5_synaptic_only`, and relaunched fresh with `learn_dynamics: true` plus the same rank-4 signed LoRA and `alpha: -0.5`. Suite/trainer/watcher PIDs 873692/874029/873693 on GPU 0. The GPU-1 job stayed up.

## [2026-09-21] query | Spectral logs through 266M frames

- Read `spectral/` and `spectral_preconditioning/` from `00_spectral_r4_a-0p5_k4_seed42` through frame 266,207,232. The preconditioner is active and nearly constant: multiplier mean 1, min/max about 0.060/1.884, target cosine about 0.76, target norm ratio about 0.15 after frame 13M. Effective `sigma_max` stays within 0.04% of the anatomical baseline 3.643. Delta RMS is 0.0145. Recorded in `docs/wiki/analyses/spectral-preconditioning-266m.md`.

## [2026-09-22] implement | Add unified probabilistic plasticity

- Added isolated `latent_probabilistic` adaptation with one zero-initialized \(N\times(3+4r)\) neuron state, fixed symmetry-breaking anchors, direct intrinsic readout, signed conditional efficacy on observed edges/nonedges, Bernoulli topology, learned `tau`, and a dual-controlled Gaussian-plus-topology information objective. The MaleCNS audit has no calibrated edge-existence probability, so the sole adjacency prior knob is `topology_prior_error_rate`.
- Added one shared anatomical-plus-candidate CSR cache. Rank 0 selects nonedges by weighted Gumbel-top-k and broadcasts exact canonical IDs. Triton regenerates lane-specific hard gates from compact rollout topology IDs and canonical edge IDs in forward, transpose hidden backward, conditional-value backward, and straight-through topology-logit backward. No per-environment graph or persistent edge-by-environment mask is created.
- Routed int64 topology IDs through experience storage, rollout accumulation, leader/follower handling, shuffle, recurrent PPO minibatches, likelihood recomputation, inference, and full checkpoint state. Added full-information dual updates, candidate-before-SVD update ordering, exact DDP support/dual/SVD synchronization, and expected-operator spectral monitoring/preconditioning of only pairwise latent slices.
- Added `SimToolRealConnectomeProbabilisticSAPG`, the 1,952-cell smoke contract, all requested `probabilistic_plasticity/*` telemetry, sampled/MAP inference, a matched benchmark contract, and focused CPU/CUDA/two-rank tests. Sixteen new tests passed; legacy CPU and Triton regressions passed 60 and 12 tests respectively.
- The final contention-sensitive training-stack benchmark measured signed-LoRA versus independent probabilistic PPO throughput at 35,947 versus 33,434 observations/s. Group size 32 measured 29,775 observations/s, so the default remains independent group size 1. Detailed equations, implementation boundaries, memory/timing values, and caveats are recorded in `docs/wiki/analyses/probabilistic-plasticity.md`.

## [2026-09-22] launch | Replace GPU-1 non-spectral job with probabilistic plasticity

- Stopped only the non-spectral intrinsic-plus-signed-LoRA suite/trainer/watcher PIDs 2849846/2850208/2849848 on physical GPU 1. Its last reported point was epoch 32,189 / frame 6,328,418,304. The output tree, TensorBoard events, and milestone artifacts were left in place. The GPU-0 spectral suite/trainer/watcher PIDs 873692/874029/873693 stayed up and continued logging.
- Added `configs/connectome/suites/prob_r4_a-0p5_100b.yaml` and `configs/connectome/evaluation/prob_r4_a-0p5_100b_milestones.yaml`. The suite copies the replaced 12,288-environment leader-follower 100B contract and the live spectral contract: enabled, `alpha: -0.5`, floor ratio `0.001`, refresh every PPO update. Plasticity is `latent_probabilistic` rank 4, group size 1, 33,720 candidate nonedges, and 1.0 nat/neuron.
- Launched a fresh run (`checkpoint.mode: none`) as experiment `00_prob_r4_a-0p5_k4_seed42` on GPU 1. Suite/trainer/watcher PIDs 491552/491846/491553. `CUDA_VISIBLE_DEVICES=1`. Summaries are linked from `train_dir/tensorboard_6008/00_prob_r4_a-0p5_k4_seed42`. Epoch 2 completed at 60,377 total FPS with finite reported metrics. Worst-case checkpoint basename for this short name is 62 bytes.
