...
For the compact fixed-input profile, 98 of the 157 descending input ports are directly driven by robot context or goal channels. An all-descending readout is valid as a performance experiment, but gives the learned head a strong shortcut around the recurrent pathway. A clean companion control is motor plus the 79 descending cells not directly driven by that profile (the 20 non-port descending cells plus 59 unused ports), for 214 cached features. Learned-adapter profiles can drive all 157 ports, so their no-direct-input hybrid reduces to the 20 non-port descending cells plus motor neurons.
