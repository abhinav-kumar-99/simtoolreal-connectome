...
Important confound: reading all descending cells also exposes the 53 goal/previous-action-driven cells. Their final states are not raw inputs: on every neural update they combine the persistent external drive with the prior state, leak, recurrent current, bias and tanh nonlinearity. Nevertheless, they have a one-update path from external value to readout and can retain a strong monotonic encoding without requiring multi-hop CNS propagation. Compare motor-only, descending-plus-motor, and descending-plus-motor excluding those 53 directly driven cells (1,261 remaining descending cells), with blank/shuffled-camera controls. Exclusion is therefore a conservative short-path ablation, not a claim that driven-cell states are unprocessed. No architecture or running job was changed for this explanation.
...
Exposing all 165,122 states would also expose the driven L1/L2, proprioceptive, goal and previous-action cells. Those states have undergone recurrent dynamics, but reading them permits a shallow path that need not use the intervening multi-hop connectome. At the current 2,304-environment, 16-step rollout size, caching all float32 states would require about 22.7 GiB, versus about 19.0 MiB for 135 motor states, before gradients and other PPO/simulator buffers. The output-layer parameter count is not the main cost; retaining one feature vector per environment and rollout step is.

A better staged comparison is the existing motor population versus motor plus the 1,261 non-driven descending cells. That hybrid would cache about 196.3 MiB at the same rollout size and preserves a meaningful intermediate-control population without exposing the shortest goal/previous-action-to-readout route. To test whether the entire CNS contains useful distributed information, use a fixed anatomical pooling or fixed random projection before caching rather than concatenating every state. Such compression keeps the reservoir frozen and avoids BPTT, but requires blank/shuffled-camera and driven-cell-exclusion controls to distinguish distributed CNS processing from shallow transformed-input features.

One concrete fixed pooling design is to exclude every externally driven sensory and descending input cell as a conservative ablation, assign each remaining neuron to a fixed annotation-derived group, and reduce the final tanh state with two channels per group: mean positive activity and mean negative activity. For group `G_k`, these are `mean(ReLU(h_i))` and `mean(ReLU(-h_i))` for neurons `i` in `G_k`. Keeping the two signs separate avoids cancellation, while using a mean prevents large populations from dominating only because they contain more cells. The reduction can be implemented as a GPU segment/scatter sum with fixed group IDs and therefore adds no trainable input projection or gradient path through the CNS. A companion inclusive variant can retain driven-cell pools; comparing it with the exclusion variant measures whether their processed but short-path states are useful.

For the current full artifact, a concrete candidate groups optic cells with valid column coordinates by `(type, side, 4x4 coarse hex bin)` and other cells by `(superclass, side, somaNeuromere-or-unknown)`. After excluding the 4,075 externally addressable sensory/descending cells, the pinned annotations yield 322 optic groups and 258 other groups. Two signed channels produce 1,160 pooled features; concatenating the raw 135 foreleg motor values gives 1,295 cached reservoir features, about 182.1 MiB for 2,304 environments and a 16-step rollout. The 32-dimensional SAPG embedding is appended at readout time. This grouping is a proposed starting point, not an empirically validated representation.

The 1,952-neuron artifact can use an unpooled descending-plus-motor readout directly. It contains 177 cells annotated `descending_neuron` and all 135 selected foreleg motor cells, for 312 unique reservoir features and 344 readout inputs after the 32-dimensional SAPG embedding. Its saved `descending_indices` array contains only the 157 descending input ports, all of which are members of the 177-cell anatomical population; using that array alone would omit 20 descending cells. Therefore implementation should add a separate readout-population array rather than redefining the input-port array. At 12,288 environments and a 16-step rollout, 312 float32 cached features use about 234 MiB versus about 101.3 MiB for the current 135-cell motor cache.

For the compact fixed-input profile, 98 of the 157 descending input ports are driven by robot context or goal channels. After the recurrent updates their states are processed mixtures rather than copies of those values, so an all-descending readout is a valid performance experiment. It still gives the learned head a much shorter input-to-action route than motor-only readout. A clean companion control is motor plus the 79 descending cells not driven by that profile (the 20 non-port descending cells plus 59 unused ports), for 214 cached features. Learned-adapter profiles can drive all 157 ports, so their no-direct-input hybrid reduces to the 20 non-port descending cells plus motor neurons.
