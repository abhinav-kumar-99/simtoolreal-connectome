...
| `successes` | Mean over the retained 2,048-environment block 5 | Exploitation-block version of average completed-episode successes. |
| `successes_median`, `successes_max` | Median and maximum over block 5 | Distribution diagnostics for that exploitation block. |
| `successes_per_block/block_0` ... `block_5` | Mean over each 2,048-environment SAPG exploration block | Exploration diagnostic. Block 0 has the largest entropy-reward coefficient; block 5 has zero. |

The observer filters the ordinary `successes` tensor to block 5 but computes `success_ratio` inside the environment before filtering, across all six blocks. This is why `successes / 50` need not equal `success_ratio`. With 2,048 environments, block means move in increments of 1/2,048; all-environment means move in increments of 1/12,288.
