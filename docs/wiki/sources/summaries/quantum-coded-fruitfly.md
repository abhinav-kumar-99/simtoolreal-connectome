...
First test a structured adapter: fixed population codes for normalized position and signed velocity, annotation-based masks, and modest learned gains/mixing or constrained residuals. Keep robot joint/group correspondence explicit. Position, directional motion and vibration-sensitive FeCO populations have physiological support ([Mamiya et al.](https://pmc.ncbi.nlm.nih.gov/articles/PMC6481666/)), but broad sensory annotations do not identify exact claw/hook tuning. Geometry/context need their own documented route; do not discard observations or relabel position proxies as touch. Adding contact channels is a separate observation/deployment change.
