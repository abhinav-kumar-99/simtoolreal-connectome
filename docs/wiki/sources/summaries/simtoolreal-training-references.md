...
Reinspection of the archived release YAML confirms exploration scale 0.005, force scale 2, force probability range [0.001, 0.1], decay 0.99, decay interval 0.08, and forces only when lifted. Torque and velocity-impulse settings are absent; the environment loader defaults their scales to zero. `configs/connectome/suites/adaptation_1b_release_settings.yaml` explicitly applies these settings in a separate output directory. It is prepared but not launched and still uses the existing 12k update geometry. Reproduce its configuration through `scripts/run_connectome_suite.py --config configs/connectome/suites/adaptation_1b_release_settings.yaml` with the repository `.venv/bin/python` after choosing the final comparison schedule.
...
The original 24,576-environment, 98,304-minibatch run failed inside the central critic forward pass. Since environment count and minibatch were reduced together, evidence does not isolate a requirement to reduce the effective minibatch. Prefer keeping the released logical rollout and optimizer schedule while microbatching actor and critic with sample-weighted gradient accumulation. Preserve SAPG's augmented dataset and enlarged last minibatch, recurrent sequence boundaries, normalization statistics, one gradient clip/Adam step per logical minibatch, and the existing KL scheduler cadence. This requires implementation and numerical validation; no accumulation support was added in this audit.
