...
Both resolved configurations report `operator_backend: triton_fused`, `weight_mode: neuron_gains`, and the intended update geometries. Initial epochs were approximately 4.1--4.5 seconds for new timing and 1.9--2.1 seconds for old timing, but rose to about 10.8 and 5.2 seconds after the first environment-reset boundary. The completed 1B jobs would extrapolate to roughly 14--15 days, while the early live rate after this reset boundary would take closer to 30 days. Treat both as provisional and recompute throughput after the first 250-million-frame milestone rather than relying on startup timing.
