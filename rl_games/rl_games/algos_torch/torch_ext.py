...
def policy_kl(p0_mu, p0_sigma, p1_mu, p1_sigma, reduce=True):
    """KL(N0 || N1), without squaring absolute scales or adding biased epsilons.

    Use float64 intermediates so every finite float32 mean/positive scale can
    be normalized before squaring, including subnormal scales. expm1 avoids
    cancellation for nearly identical distributions. Invalid inputs produce
    +inf per sample, which the adaptive scheduler treats as a reduction signal.
    """
    output_dtype = torch.promote_types(p0_mu.dtype, p0_sigma.dtype)
    output_dtype = torch.promote_types(output_dtype, p1_mu.dtype)
    output_dtype = torch.promote_types(output_dtype, p1_sigma.dtype)
    if output_dtype in (torch.float16, torch.bfloat16):
        output_dtype = torch.float32
    mu0, s0, mu1, s1 = (
        tensor.to(torch.float64)
        for tensor in (p0_mu, p0_sigma, p1_mu, p1_sigma)
    )
    valid = (torch.isfinite(mu0) & torch.isfinite(mu1)
             & torch.isfinite(s0) & torch.isfinite(s1) & (s0 > 0) & (s1 > 0))
    log_ratio = torch.log(s0 / s1)
    scale_term = 0.5 * torch.expm1(2.0 * log_ratio) - log_ratio
    mean_term = 0.5 * ((mu0 - mu1) / s1).square()
    terms = scale_term + mean_term
    terms = torch.where(valid & torch.isfinite(terms), terms, torch.full_like(terms, float('inf')))
    kl = terms.sum(dim=-1).to(output_dtype)
    if reduce:
        return kl.mean()
    else:
        return kl
